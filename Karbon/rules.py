...
def total_revenue(data: dict, financial_index):
    ...
    net_revenue = pnl_data.get("netRevenue", 0.0)
...
def total_borrowing(data: dict, financial_index):
    ...
    # Check if "bs" exists in the financial entry
    bs_data = financial_entry.get("bs", {})

    # Sum long-term and short-term borrowings
    total_borrowings = bs_data.get("longTermBorrowings", 0.0) + bs_data.get("shortTermBorrowings", 0.0)

    # Get total revenue from the total_revenue function
    ...
    if total_revenue_value != 0:
        ...
        borrowing_to_revenue_ratio = 0.0
...
def total_revenue_5cr_flag(data: dict, financial_index):
    ...
    if total_revenue_value >= 50000000:  # Assuming 50 million is the threshold
        return FLAGS.GREEN
    else:
        return FLAGS.RED
...
def iscr(data: dict, financial_index):
    ...
    # Check if "pnl" exists in the financial entry
    pnl_data = financial_entry.get("pnl", {})

    # Calculate ISCR
    profit_before_interest_tax_depreciation = pnl_data.get("profitBeforeInterestTaxDepreciation", 0.0)
    interest_expenses = pnl_data.get("interestExpenses", 0.0)

    # Avoid division by zero
    if interest_expenses != 0:
        iscr_value = (profit_before_interest_tax_depreciation + 1) / (interest_expenses + 1)
    else:
        iscr_value = 0.0
